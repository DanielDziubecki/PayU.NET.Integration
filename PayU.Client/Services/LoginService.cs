...
namespace PayU.Client.Services
{
    public class LoginService : ILoginService
    {
        public async Task<string> GetToken(LoginDto loginDto)
        {
            ...
            {
                var response = await httpClient.PostAsync(httpClient.BaseAddress, new FormUrlEncodedContent(new List<KeyValuePair<string, string>>()
                {
                ...
                {
                    var deserializedResponse = new JavaScriptSerializer().Deserialize<Dictionary<string, string>>(contents);
